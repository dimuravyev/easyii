 ...
 * @method static array catItems($id_slug, $options = [], $includeChildren = false)
 ...
 * @method static mixed catLast($id_slug, $limit, $where = null, $includeChildren = false) Get last articles
 ...
 * @method static array catChildrenItems($id_slug, $options = []) Get list of articles as ArticleObject objects
 */
...
class Article extends \yii\easyii\components\API
{
    ...
    public function api_catItems($id_slug, $options = [], $includeChildren = false)
    {
        if (!$category = $this->api_cat($id_slug)) {
            return null;
        }
        $categoryId = $includeChildren
            ? $category->model->getChildrenCategoryQuery()->select('category_id')->column()
            : $category->id
        ;

        $options['where'] = isset($options['where'])
            ? ['and', ['category_id' => $categoryId], $options['where']]
            : ['category_id' => $categoryId]
        ;
        return $this->api_items($options);
    }
    ...
    public function api_catLast($id_slug, $limit, $where = null, $includeChildren = false)
    {
        if (!$parentCat = $this->api_cat($id_slug)) {
            return null;
        }
        $categoryIdList = $includeChildren
            ? $parentCat->model->getChildrenCategoryQuery()->select('category_id')->column()
            : $parentCat->id
        ;

        $where = $where
            ? ['and', ['category_id' => $categoryIdList], $where]
            : ['category_id' => $categoryIdList]
        ;
        return $this->api_last($limit, $where);
    }
